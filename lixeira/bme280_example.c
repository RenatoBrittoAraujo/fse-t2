 ...
 *
 */
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev, struct bme280_data *data);
...
struct bce280_data *get_bce280_temp_press_humidity(char *ic2_bus)
{
    ...
    if (ic2_bus == NULL)
    {
        fprintf(stderr, "ERROR Missing argument for i2c bus.\n");
        return NULL;
    }

    if ((id.fd = open(argv[1], O_RDWR)) < 0)
    {
        fprintf(stderr, "ERROR Failed to open the i2c bus %s\n", argv[1]);
        return NULL;
    }
    ...
    if (rslt != BME280_OK)
    {
        fprintf(stderr, "ERROR Failed to initialize the device (code %+d).\n", rslt);
        return NULL;
    }
    ...
    if (rslt != BME280_OK)
    {
        fprintf(stderr, "ERROR Failed to stream sensor data (code %+d).\n", rslt);
        return NULL;
    }
...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev, struct bme280_data *data)
{
    ...
    if (rslt != BME280_OK)
    {
        fprintf(stderr, "ERROR Failed to get sensor settings (code %+d).", rslt);
    ...
    if (rslt != BME280_OK)
    {
        fprintf(stderr, "ERROR Failed to set sensor settings (code %+d).", rslt);
    ...
    while (1)
    {
        ...
        if (rslt != BME280_OK)
        {
            fprintf(stderr, "ERROR Failed to set sensor mode (code %+d).", rslt);
            break;
        }
        ...
        if (rslt != BME280_OK)
        {
            fprintf(stderr, "ERROR Failed to get sensor data (code %+d).", rslt);
            break;
        }

        print_sensor_data(&comp_data);
        memcpy(data, &comp_data, sizeof(struct bme280_data));
    }
